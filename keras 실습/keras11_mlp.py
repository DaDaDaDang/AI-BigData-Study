...
# YYY=np.array([range(10), range(11,21)])
XXX=np.array([range(100),range(311,411),range(1000,1100)])
YYY=np.array([range(501,601), range(622,522,-1),range(800,900)])

XXX=XXX.T # == np.transpose(XXX)==XXX.reshape(10,2)==np.swapaxes(XXX,axis1,axis2)
# XXX=np.swapaxes(XXX,0,1) #0은 첫번째 [], 1은 두번째 []
YYY=YYY.T
...
model.add(Dense(24, input_dim = 3, activation = 'relu')) #model.add(Dense(1024, input_shape=(2,), activation = 'relu'))
model.add(Dense(256))
model.add(Dense(512))
model.add(Dense(128))
model.add(Dense(128))
...
# ------------------------------------------------------------------
model.fit(x_train, y_train, epochs = 60 ,batch_size = 1, validation_data=(x_val, y_val)) #validation_data : 훈련할 때 머신이 직접 검증할 수 있도록 넣어주는 데이터셋
